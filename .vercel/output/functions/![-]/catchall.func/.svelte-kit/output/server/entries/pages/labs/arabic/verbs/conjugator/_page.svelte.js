import { c as create_ssr_component, v as validate_component, d as escape, e as each, j as null_to_empty } from "../../../../../../chunks/ssr.js";
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<h1 class="page-title svelte-1ob9mkf" data-svelte-h="svelte-1anl4c">Sarf Lab:</h1> ${validate_component(Search, "Search").$$render(
    $$result,
    {
      deliver: handleDelivery,
      show: true,
    ...
    {},
    {}
  )} <hr> ${chosenVerb ? `<div class="built svelte-1ob9mkf"><div class="chosen svelte-1ob9mkf"><h1 class="svelte-1ob9mkf"><p>Form ${escape(chosenVerb.v_form)}</p> ${each(chosenVerb.v_word.split("/"), (word, index) => {
    return ` <span>${escape(word.replace(" ", ""))}</span> ${index === 0 ? `${escape(" / ")}` : ``}`;
  })}</h1></div> ${chosenLetters && chosenLetters.length > 0 ? `<div class="inputted svelte-1ob9mkf"><h1 class="svelte-1ob9mkf">${each(chosenLetters, (letter) => {
