...
<div class="set">
	...
	<div class="skills">
		{#each set.skills as skill}
			<div
				class="skill"
				class:passing={$user.bank_skills &&
					$user.bank_skills.filter((bs) => bs.skill_id === skill.id && bs.status === 100).length >
						0}
			>
				<div class="content">
					<span
					...
					>
						<div class="check text-center">
							{#if $user.bank_skills && $user.bank_skills.filter((bs) => bs.skill_id === skill.id && bs.status === 100).length > 0}
								<svg
					...
					{#if $user && $user.admin}
						<div class="arrows">
							<div class="i fa fa-arrow-left" on:click={() => moveSkill(skill, 'left')} />
							<div class="i fa fa-arrow-right" on:click={() => moveSkill(skill, 'right')} />
						</div>
